...
resource "aws_api_gateway_integration" "lambda" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "${aws_lambda_function.lambda-nodejs.invoke_arn}"
}
...
resource "aws_api_gateway_deployment" "lambdagateway" {
  depends_on = [
...
resource "aws_lambda_permission" "apigw" {
  ...
  function_name = "${aws_lambda_function.lambda-nodejs.function_name}"
...
output "base_url" {
  value = "${aws_api_gateway_deployment.lambdagateway.invoke_url}"
}